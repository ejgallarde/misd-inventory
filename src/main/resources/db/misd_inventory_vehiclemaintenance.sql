...
-- destroyed `vehiclemaintenance` and everything cascading from it, with no prompt.
...
CREATE TABLE `vehiclemaintenance` (
  ...
  `OdometerAtService` int DEFAULT NULL,
  `ServiceType` varchar(100) DEFAULT NULL,
  `Cost` decimal(10,2) DEFAULT NULL,
  PRIMARY KEY (`LogID`),
  KEY `VehicleID` (`VehicleID`),
  CONSTRAINT `vehiclemaintenance_ibfk_1` FOREIGN KEY (`VehicleID`) REFERENCES `fleetvehicles` (`VehicleID`) ON DELETE CASCADE
