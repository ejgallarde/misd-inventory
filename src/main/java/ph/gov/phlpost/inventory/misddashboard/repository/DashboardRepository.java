...
public interface DashboardRepository extends Repository<AgingEquipmentReport, String> {
    ...
    @Query(value = "SELECT " +
            ...
            "WHERE " +
            "a.DeploymentStatus IN ('Missing / Unaccounted', 'On Loan', 'In Transit') " +
            "OR a.MaintenanceHealthStatus IN ('Degraded', 'Under Repair', 'Awaiting Parts', 'Beyond Economic Repair (BER)') "
            +
    ...
    @Query(value = "SELECT COUNT(*) FROM Assets WHERE " +
            "DeploymentStatus IN ('Missing / Unaccounted', 'On Loan', 'In Transit') " +
            "OR MaintenanceHealthStatus IN ('Degraded', 'Under Repair', 'Awaiting Parts', 'Beyond Economic Repair (BER)') "
            +
    ...
    @Query(value = "SELECT COUNT(*) FROM Assets WHERE DeploymentStatus IN ('Missing / Unaccounted', 'On Loan', 'In Transit')", nativeQuery = true)
    ...
    @Query(value = "SELECT COUNT(*) FROM Assets WHERE LifecycleStatus IN ('End of Life (EOL)', 'Decommissioned / Retired', 'Disposed', 'Sold')", nativeQuery = true)
    long countLifecycleIssues();
