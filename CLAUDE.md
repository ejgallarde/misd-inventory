...
The original **Properties module** (land assets + buildings/facilities — `PropertiesController`/`RealEstateProperty`/`properties.html`) still exists in the codebase but is **intentionally unlinked from the dashboard** for this deployment: no nav tab, no dashboard model wiring, and its backing `realestateproperties` table does not exist in `dar_inventory`. Do not wire it back into `dashboard.html`/`MainDashboardController` without also adding that table back to the DAR schema.
...
### Document storage
`DocumentService` validates uploads (size/extension/category-per-reference-type, configured via `document.upload.*` properties) and delegates actual byte storage to `DocumentStorageService`, which switches between filesystem and MinIO based on `storage.mode`. Reference IDs are resolved per domain (`resolveVehicleStorageId`/`resolvePropertyStorageId`/`resolveSurveyAssetStorageId` prefer plate number / title number / asset tag or serial number over the numeric PK) before building the storage path. `DocumentService`'s reference-type map (`allowedCategoriesByReferenceType`, `toStorageFolder`, `resolveStorageEntityId`) is a fixed switch over literal reference-type strings (`IT_EQUIPMENT`, `VEHICLE`, `PROPERTY`, `SURVEY_ASSET`) — adding a domain means adding a case in all three places, not just injecting a new `@Value` category CSV.
...
- Entity ID strategy is mixed: `Asset`/`Personnel`/PSGC entities use natural/business-key `String` `@Id`s; `FleetVehicle`/`SurveyAsset`/`RealEstateProperty`/`Document`/audit-log entities use `IDENTITY` `Integer`. Follow whichever convention the entity you're touching already uses.
