...
MISD Master Asset Registry — a Spring Boot 4.1 / Java 17 monolith (Thymeleaf + Bootstrap 5 + jQuery/DataTables/Select2), originally built for PHLPost's MISD division. **This branch is a dedicated DAR (Department of Agrarian Reform) deployment**, customized to track three asset domains: **IT equipment**, **fleet vehicles**, and **survey equipment** (land-survey instruments — GNSS receivers, total stations, etc.), plus a document-attachment system shared across all three. It runs against its own `dar_inventory` database (`db/dar_inventory_schema.sql`), separate from the original MISD `misd_inventory` schema.

The original **Properties module** (land assets + buildings/facilities — `PropertiesController`/`RealEstateProperty`/`properties.html`) has been **removed entirely** from this branch. Land, building, and facility assets are out of scope for `temp-dar-split-branding`; do not reintroduce that module without a deliberate decision to bring it back, including adding a `realestateproperties` table to the DAR schema.
...
# Run the app locally (needs MySQL at localhost:3306/dar_inventory — see Local setup below)
...
- `spring.datasource.url` points at `jdbc:mysql://localhost:3306/dar_inventory` with `spring.jpa.hibernate.ddl-auto=none` — schema must already exist. Create it with `src/main/resources/db/dar_inventory_schema.sql`. `src/main/resources/db/README.md` explains what is in that folder; `misd_inventory_*.sql` are reference dumps of the original MISD schema (kept for documentation, not used by this branch) and `migration_*.sql` are dated corrective scripts against that original schema. **`MisdInventoryDashboardApplicationTests` (`@SpringBootTest`) boots the full context and will fail without a reachable MySQL instance matching this config** — every other test avoids Spring entirely (see Testing below).
...
### Package layout
`ph.gov.phlpost.inventory.misddashboard` — flat by responsibility, not by domain: `controller/`, `service/`, `repository/`, `model/`, `config/`, `util/`. Each of the three active asset domains (IT/Fleet/Survey Assets) gets one controller and one service, plus a workflow-helper `@Component` (`AssetWorkflowHelper`, `FleetWorkflowHelper`, `SurveyAssetWorkflowHelper`) for status badges and lifecycle-action predicates. `SurveyAssetController`/`SurveyAssetService`/`SurveyAssetRepository`/`SurveyAssetWorkflowHelper` were modeled directly on the Fleet module (single table, no sub-catalog, three independent status axes, lock-once identity fields, a direct custodian column rather than a join table) — use Fleet as the template when extending Survey Assets.

### Blank input, error handling, and audit keys
`config/WebBindingAdvice` registers a global `StringTrimmerEditor(true)`, so every **form-bound** blank field arrives as `null`, not `""`. This matters because several optional columns are `UNIQUE` (`Assets.SerialNumber`, `FleetVehicles.PlateNumber`/`BodyNumber`/`EngineNumber`/`ChassisNumberVIN`) and MySQL allows many `NULL`s in a unique index but only one `''`. **JSON request bodies bypass this** — Jackson never touches a `WebDataBinder` — so `/assets/update` normalizes blanks itself in `normalizeBlankOptionalFields`, and the browser side does the same via `MISDCommon.serializeFormToPayload`. Keep both halves in mind when adding an endpoint that takes `@RequestBody`.
...
- Dropdown option lists in `application-dropdowns.properties` must match the literal values in native `@Query` strings. A mismatch was corrected on 2026-09-01: the fleet problem queries now accept `'Missing'` and `'Stolen'` (the values actually written) alongside the legacy `'Missing/Stolen'`.
...
- Entity ID strategy is mixed: `Asset`/`Personnel`/PSGC entities use natural/business-key `String` `@Id`s; `FleetVehicle`/`SurveyAsset`/`Document`/audit-log entities use `IDENTITY` `Integer`. Follow whichever convention the entity you're touching already uses.
